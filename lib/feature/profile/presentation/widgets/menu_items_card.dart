...
class MenuItemsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _buildMenuItem(context, 'تعديل الملف الشخصي', Icons.edit_outlined, () {}),
          _buildMenuItem(context, 'مكافآتك', Icons.card_giftcard_outlined, () {}),
            ...
            showDialog(
              ...
              builder: (BuildContext dialogContext) {
                return AlertDialog(
                  ...
                  title: const Text('تأكيد تسجيل الخروج', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget _buildMenuItem(BuildContext context, String title, IconData icon, VoidCallback onTap, {bool isLogout = false}) {
    final color = isLogout ? AppColors.error : AppColors.textPrimary;
    return ListTile(
      leading: Icon(icon, color: color),
      title: Text(title, style: TextStyle(color: color, fontWeight: FontWeight.w500)),
      trailing: isLogout ? null : const Icon(Icons.arrow_forward_ios, size: 16, color: AppColors.textSecondary),
      onTap: onTap,
    );
  }
}
